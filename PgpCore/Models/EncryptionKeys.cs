...
namespace PgpCore.Models
{
    public class EncryptionKeys : IEncryptionKeys
    {
        ...
        /// <exception cref="ArgumentNullException">Invalid Pass Phrase. <paramref name="passPhrase"/></exception>
        ...
        /// <exception cref="ArgumentException">Public key not found. Private key not found. Missing password</exception>
        /// <exception cref="ArgumentNullException">Invalid Pass Phrase. <paramref name="passPhrase"/></exception>
        /// <exception cref="FileNotFoundException">Public Key file does not exist.</exception>
        ...
        public EncryptionKeys(IEnumerable<string> publicKeys, string privateKey, string passPhrase)
        {
            ...
            var publicKeyStrings = publicKeys.ToArray(); // Avoid multiple enumeration
            foreach (var publicKey in publicKeyStrings)
                if (string.IsNullOrEmpty(publicKey))
        ...
        public EncryptionKeys(IEnumerable<FileInfo> publicKeyFiles, FileInfo privateKeyFile, string passPhrase)
        {
            ...
            var publicKeyFileInfos = publicKeys.ToArray(); // Avoid multiple enumeration

            foreach (var publicKeyFile in publicKeyFileInfos)
            {
                if (publicKeyFile == null)
        ...
        {
            // Avoid multiple enumerations of 'publicKeyFilePaths'
            var publicKeyStreamArray = publicKeyStreams.ToArray();

            if (privateKeyStream == null)
                throw new ArgumentException("PrivateKeyStream");
            if (passPhrase == null)
                throw new ArgumentNullException(nameof(passPhrase), "Invalid Pass Phrase.");
            foreach (var publicKeyStream in publicKeyStreamArray)
                if (publicKeyStream == null)
                    throw new ArgumentException("PublicKeyStream");
        ...
        {
            var publicKeyStrings = publicKeys.ToArray();
            foreach (var publicKey in publicKeyStrings)
                if (string.IsNullOrEmpty(publicKey))
                    throw new ArgumentException(nameof(publicKey));
        ...
        public EncryptionKeys(IEnumerable<FileInfo> publicKeyFiles)
        {
            ...
            foreach (var publicKeyFile in publicKeys)
            {
        ...
        public PgpPrivateKey FindSecretKey(long keyId)
        {
            if (SecretKeys == null)
                throw new ArgumentNullException(
                    "No private keys found. These should be provided in EncryptionKeys constructor.");
        ...
        private static PgpPrivateKey ReadPrivateKey(PgpSecretKey secretKey, string passPhrase)
        {
                    ...
                    null) // Should only be run as the last step during construction!
        {
            ...
            else
            {
                ...
                _publicKeyRingsWithPreferredKey = new Lazy<IEnumerable<PgpPublicKeyRingWithPreferredKey>>(() =>
                    publicKeyRings.Select(keyRing => new PgpPublicKeyRingWithPreferredKey(keyRing)).ToArray());
