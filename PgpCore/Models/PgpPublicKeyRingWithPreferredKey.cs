...
namespace PgpCore.Models
{
    ...
    public class PgpPublicKeyRingWithPreferredKey
    {
        ...
        private readonly Lazy<PgpPublicKey> _defaultEncryptionKey;
        private readonly Lazy<IEnumerable<PgpPublicKey>> _encryptionKeys;
        ...
        public PgpPublicKeyRingWithPreferredKey(PgpPublicKeyRing publicKeyRing)
        {
            ...
            _encryptionKeys = new Lazy<IEnumerable<PgpPublicKey>>(() =>
                PgpPublicKeyRing.GetPublicKeys().Where(key => key.IsEncryptionKey));
        }
        ...
        {
            PreferredEncryptionKey = _encryptionKeys.Value.FirstOrDefault(key => key.KeyId == keyId) ??
                                     PreferredEncryptionKey;
        }
    }
}